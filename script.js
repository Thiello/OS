        ...
        img.onload = function () {
            ...
            itens.forEach((item, index) => {
                if (index < itemCoords.length) { // Limita a cinco itens
                    // Exibe quantidade, descrição, valor unitário e valor total
                    ...
                    doc.text(`R$ ${item.valorUnitario.toFixed(2)}`, itemCoords[index].xValorUnitario, itemCoords[index].yValorUnitario);
                    doc.text(`R$ ${item.valorTotal.toFixed(2)}`, itemCoords[index].xValorTotal, itemCoords[index].yValorTotal);
                }
            });

            // Exibe o valor total no final
